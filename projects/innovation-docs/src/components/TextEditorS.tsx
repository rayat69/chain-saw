...
import Quill, { TextChangeHandler,DeltaStatic } from 'quill'
...
const TextEditor = () => {
  ...
  const [quills, setQuills] = useState<QuillInstance[]>()
  // Mui styled classes
  ...
  const textChangeHandler = useCallback((idx: number): TextChangeHandler => {
    ...
    console.log('creating text-changer')
  ...
  const initializeQuill = useCallback(
    (
      ...
      return { editor: q, handler }
    },
    [textChangeHandler]
  )

  const docLoader = useCallback(
    (documents: DeltaStatic[]) => {
    ...
    [initializeQuill, wrapperRef, toolbarRef]
  )
  console.log('gas')
  ...
  useEffect(() => {
    ...
    socketDev.once('quill-editor-load-document', docLoader)
    ...
    return () => {
      ...
      socketDev.off('quill-editor-load-document', docLoader)
      console.log('Real cleanup')
    }
  }, [documentId, initializeQuill, docLoader, wrapperRef, toolbarRef])

  // Save document
  useEffect(() => {
    if (!socketDev || !quills || quills.length === 0) return
    // ;(() => {
    if (editorState.typing || editorState.saved) {
      return
    }
    const contents = quills.map(quill => quill.editor.getContents())
    socketDev.emit('quill-editor-save-document', contents)
    console.log('Saving 2:xxx', contents)
    setEditorState(prev => ({ ...prev, saved: true }))
    // })()
  }, [quills, editorState.saved, editorState.typing])
  ...
  useEffect(() => {
    ...
    const handler = (delta: DeltaStatic, index: number) => {
      quills[index].editor.updateContents(delta, 'api')
    }
  ...
  }, [quills, classes.editor, classes.toolbar])
